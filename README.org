...
The heart of the parsing is done by iterating through the list of =random-table/text-replacer-functions=, sequentially calling each one and replacing text that matches the function.  The =random-table/create-text-replacer-function= provides assistance in composing one of these functions.
